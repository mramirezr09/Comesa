...
//Creación de la lista de asistencia masiva
$query= " SELECT
			...
			LEFT JOIN [PSA.TipoPago]t7 ON t1.FK_IdTipoPago=t7.Pk_IdTipopago
			--where PK_IdUsuario='DS541'
			where t1.FK_IdProyecto=3 and t1.FK_IdTipoPago in (2,3) and t1.FK_IdPerfil=2 
			--and PK_IdUsuario not in ('DS107','DS133','SEGS1648','SEGS1362')";
      // print_r($query);
...
$query1= "  SELECT 
			...
			(select CASE 
				...
				(select CASE 
							...
							THEN 'Completada' ELSE 'Incapacidad' END) as 'Jornada Total'
				
				
				FROM [PSA.Registro_Asistencia] t1
				INNER JOIN [PSA.UsuarioAsistencia]t2 on t2.PK_IdUsuario=t1.FK_IdUsuario
				INNER JOIN [PSA.Puesto]t3 on t2.FK_IdPuesto=t3.PK_IdPuesto
				 where t1.FK_IdUsuario='$idm' and CAST(t1.Fecha_Asistencia as date) between '$inicio' and '$fin' group by t1.FK_IdUsuario,t1.fecha_c
				,convert( char(10),t1.Fecha_Asistencia,120 ),t2.Nombre, t2.Apallido_Materno, t2.Apallido_Paterno, t1.hora_1, t3.Nombre_Puesto, CAST( t1.Fecha_Asistencia AS DATE )
...
// Generación de QR
			 ...
			 $pdf->Ln(25);   
		$pdf->Cell(50);
		$pdf->Cell(85,7,"No se han encontrado registros para mostrar",0,0,'C');
		 $pdf->Ln(85);   
	}
	...
	$folder ="Web/QR/temp/";
	$dir = $server.$folder;
	$nQR=$efin.'_'.$rfc.'_asistenciaQR.png';
	...
	$contenido = $empleado.' con RFC: '.$rfc.' Puesto: '.$puesto.' ha aceptado los terminos de uso  de la clausula 9 de su contrato para generar este documento de asistencia el día '.$efin; //Texto
...
//$pdf->Cell(130,5,utf8_decode($supervisor),0,0,'C');

	  } 

$pdf->Output("finalito.pdf", "D");
  // $pdf->Output();
